export const VIEW_META: Record<string, { title: string; crumb: string }> = {
  "/": { title: "Dashboard Overview", crumb: "Overview" },
  "/business": { title: "Business Overview", crumb: "Business Overview" },
  "/orders": { title: "Gift360 Order Information", crumb: "Orders" },
  "/reports": { title: "Reporting & Self-Service Queries", crumb: "Reports" },
  "/customers": { title: "Suspicious Customer Management", crumb: "Customers" },
  "/vouchers": { title: "Voucher Management", crumb: "Vouchers" },
  "/approvals": { title: "Approvals", crumb: "Approvals" },
  "/audit": { title: "Audit Logging", crumb: "Audit Log" },
};
